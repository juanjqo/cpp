...
DQ DQ_SerialManipulatorMDH::get_joint_twist(const int &ith) const
{
    return _get_w(ith);
}

double DQ_SerialManipulatorMDH::get_generalized_joint_force(const DQ &wrench, const int &ith) const
{
    const int joint_type = int(mdh_matrix_(4,ith));
    if(joint_type == 0)
    {
